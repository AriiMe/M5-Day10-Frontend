...
class CommentsList extends React.Component {
  ...
  renderComments = () => {
    if (this.state.status === true) {
      ...
      return (
        reviews && (
          <Card className="m-0 p-0 card-dark">
  ...
  reviewItem = (review) => {
    return (
      <ListGroup.Item
        key={review._id}
        className="d-flex flex-column justify-content-between p-1 pb-0"
      >
        ...
        <div className="d-flex justify-content-between pr-4">
          <p>{review.comment}</p>{" "}
          <p>
            {[...Array(parseInt(review.rate))].map((e, i) => (
              <i className="text-warning font-weight-bold" key={i}>
                ☆
              </i>
            ))}
          </p>
        </div>
        <small className="text-muted mr-1 d-flex justify-content-between">
          <Moment format="D MMM YYYY" withdate={review.createdAt} />{" "}
          <Moment format=" HH:MM " withdate={review.createdAt} />
        </small>
      </ListGroup.Item>
    );
  };
  ...
  searchResults = () => {
    return (
      <input
        type="search"
        className="form-control w-50 h-25 p-1 mb-2"
        name="search"
        id="search"
        placeholder="Search..."
        onChange={this.searchComments}
      />
    );
  };
  ...
  searchComments = (event) => {
    ...
    reviews = reviews.filter(
      (review) =>
        review.comment.toLowerCase().includes(search_key.toLowerCase()) ||
        review.author.toLowerCase().includes(search_key.toLowerCase())
    );

    this.setState({
      reviews: search_key.length > 2 ? reviews : this.book_review,
    });
  };
